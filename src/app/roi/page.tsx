...
const EVChargingCalculator: React.FC = () => {
  ...
  const calculateRevenue = () => {
    ...
    if(!formData.chargerDailyHour || !formData.electricityCost || !formData.chargingPrice || !formData.numberOfStations){
      return alert("Please Fill all Field")
    }
    if ("dual-port-level-2" == "dual-port-level-2") {
      const dailyUse = 19.2 * formData.chargerDailyHour;
      const dailyExpense = dailyUse * formData.electricityCost;
      const dailyRevenue = dailyUse * formData.chargingPrice;
      const dailyProfit = dailyRevenue - dailyExpense;
      const totalStationProfit = dailyProfit * formData.numberOfStations;
      const monthlyProfit = totalStationProfit * 30;
      const AnnualProfit = totalStationProfit * 365;
      setAnnualRevenue(AnnualProfit);
      setMonthlyRevenue(monthlyProfit);
    }

    // Start CO2 Calculation
    ...
    const milesDrivenByEVsPerChargerPerDay = formData.chargerDailyHour * 365 * 19.2; // Assumes chargers are used every day of the week

    const annualCO2Impact =
      (averageCO2EmissionsGasolineCar - averageCO2EmissionsEV) *
      milesDrivenByEVsPerChargerPerDay *
      formData.numberOfStations;
      setAnnualCO2Impact(annualCO2Impact);



    // const electricVehicleEnergyConsumption = 0.25; // kWh/mile (or your estimated value)
    // const averageCO2EmissionsEV = 0.2; // in kg of CO2 per kWh

    // const annualElectricityConsumption =
    //   formData.chargerDailyHour * 365 * formData.numberOfStations;
      
    // const annualCO2Emissions =
    //   annualElectricityConsumption * averageCO2EmissionsEV;

    // const annualCO2EmissionsGasolineCar = 2.2; // kg of CO2 per liter of gasoline (or your specific value)

    // const annualGasolineConsumption =
    //   annualElectricityConsumption / (electricVehicleEnergyConsumption / 33.7); // Assuming 1 gallon = 33.7 kWh (the energy equivalent of a gallon of gasoline)
    
    //   const annualCO2EmissionsReduction =
    //   annualGasolineConsumption * annualCO2EmissionsGasolineCar -
    //   annualCO2Emissions;

      // setAnnualCO2Impact(annualCO2EmissionsReduction);
  ...
  return (
            ...
            <div className="flex flex-col gap-y-3 ">
              ...
              <section className="shadow-lg">
                <div className="items-center justify-around	space-y-4 sm:flex sm:space-y-0 sm:space-x-4 pt-4">
                  <div
                    className={`w-full sm:w-auto $ bg-gray-600 hover:bg-gray-700 focus:ring-4 focus:outline-none focus:ring-gray-300 text-white rounded-lg inline-flex items-center justify-center px-4 py-2.5 dark:bg-gray-700 dark:hover:bg-gray-600 dark:focus:ring-gray-700`}
                  >
                    <svg
                      version="1.1"
                      id="Layer_1"
	...
	"
                    ...
                    <div className="text-left p-2">
                      <div className="-mt-1 font-sans text-2xl font-bold">
                        {monthlyRevenue.toLocaleString('en-US', { style: 'decimal', maximumFractionDigits: 2})}
                      </div>
	...
	"
                    ...
                    <div className="text-left p-2">
                      <div className="-mt-1 font-sans text-2xl font-bold">
                      {annualRevenue.toLocaleString('en-US', { style: 'decimal',maximumFractionDigits: 2})}
                      </div>
                      <div className="mb-1 text-xs">
	...
	c-34.8,6-61.6,36.8-61.6,73.6c0,41.2,33.6,74.8,74.8,74.8h243.6c41.2,0,74.8-33.6,74.8-74.8C450.4,255.6,433.2,230,408.4,217.6z"
		...
		s-3.6,4.4-6.8,8l-16,16.4v9.2h38.4v-10.8H342V286.8z"
                    ...
                    <div className="text-left p-2">
                      <div className="-mt-1 font-sans text-2xl font-bold">
                        {annualCO2Impact.toLocaleString('en-US', { style: 'decimal', maximumFractionDigits: 2})} lbs
                      </div>
                ...
                <form
                  ...
                  className="flex flex-col gap-y-6 p-8"
                >
                  ...
                  {/* <div>
                    ...
                    <select
                      name="chargerType"
                      disabled
                      value={formData.electricityCost.toString()} // Ensure the value is a string
                      onChange={handleSelectChange}
                      className="block p-3 w-full text-white rounded-md bg-gray-600 focus:outline-black h-14"
                    >
                      <option value="dual-port-level-2" defaultChecked>
                        Dual-Port Level 2
                      </option>
                  ...
                  <div>
                    <label
                      htmlFor="chargerDailyHour"
                      className="block mb-2 text-sm font-medium text-white dark:text-white"
                    >
                      Charger Utilization (Hours/daily):
                    </label>
                    <input
                      type="number"
                      name="chargerDailyHour"
                      value={formData.chargerDailyHour}
                      onChange={handleInputChange}
                      className="block p-3 w-full text-white rounded-md bg-gray-600 focus:outline-black h-14 "
                      placeholder="6"
                    />
                  ...
                  <div className="flex  pt-5 pb-2 justify-around">
                    <button
                      className={' text-white rounded-md h-12 w-[45%] bg-indigo-500'}
                      type="button"
                    ...
                    <button
                      className={` text-white rounded-md bg-cyan-800 h-12 w-[45%]`}
                      type="button"
                      onClick={resetForm}
                    >
                      Reset
